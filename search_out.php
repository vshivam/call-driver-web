...
$sql_search = "SELECT * FROM driver_pro WHERE ($car ='1') AND (city = '$user_city') AND (outstation = 1)";
$result = mysql_query($sql_search);

$query = mysql_query("SELECT count(*) FROM driver_pro WHERE ($car ='1') AND (city = '$user_city') AND (outstation = 1)");
...
if($num_rows ==0)
{
	...
	echo "</br>";
	echo "</br>";
	echo "<center> <a href=\"index.php#fragment-2\"> Go Back To Search Page </a> </center>";
...
while($row_1 = mysql_fetch_array($result))
{
	...
	for($i = 0; $i < $array_size; $i++)
	{
		...
		if($result[$i]['dist'] = 2000)
		{
			$dist_1 = "N/A";
		echo "<tr> <td> <a ".$result[$i]['link'].">". $result[$i]['name']." </a></td><td> ".$result[$i]['curr_age']."</td><td> +91 ".$result[$i]['mobile_num']." </td> <td><img style=\"padding-right:5px;\" src=\"images/rupee.png\"/>".$result[$i]['min_rate']."</td> <td>".$result[$i]['smoker'] ."</td><td> ".$result[$i]['drinker']."</td><td>".$dist_1."</td></tr>";
